...
function recorder() {
  ...
  const exec = async (args: string[]) => {
    calls.push(args);
    if (args.includes("create")) return "https://github.com/acme/x/pull/42\n";
    return "";
  };
  return { calls, exec };
}
...
test("openChangesetPrs opens one PR per entry and returns the PR numbers", async () => {
  ...
  expect(prs.get("acme/design_system")).toBe(42);
  expect(prs.get("acme/mail_box_components")).toBe(42);
...
test("PR body names the version bump and the upstream cause", async () => {
  ...
  expect(body).toContain("@acme/design");
...
test("armAll arms auto-merge for every PR", async () => {
  const { calls, exec } = recorder();
  const prs = new Map([["acme/design_system", 7], ["acme/mail_box_components", 8]]);
